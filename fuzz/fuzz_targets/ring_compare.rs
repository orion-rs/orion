...
fn ro_pbkdf2(buf1: &[u8], buf2: &[u8], hmac: ShaVariantOption) {
    ...
    ring_pbkdf2::derive(return_digest(hmac), iter as u32, &salt, &password, &mut dk_out);

    let orion_dk = dk.derive_key().unwrap();

    assert_eq!(dk_out, orion_dk);

    assert!(ring_pbkdf2::verify(return_digest(hmac), iter as u32, &salt, &password, &dk_out).is_ok());
    assert!(ring_pbkdf2::verify(return_digest(hmac), iter as u32, &salt, &password, &orion_dk).is_ok());
