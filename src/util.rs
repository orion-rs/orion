...
pub fn compare_ct(x: &[u8], y: &[u8], length: usize) -> bool {
...
fn test_compare_ct_results() {
    ...
    assert_eq!(compare_ct(&test_v_1, &test_v_2, 32), true);
    assert_ne!(compare_ct(&test_v_1, &test_v_3, 32), true);
    assert_eq!(compare_ct(&test_v_1, &test_v_4, 32), false);
}
