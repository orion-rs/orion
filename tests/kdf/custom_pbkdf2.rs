...
mod custom_test_vectors {
    ...
    fn test_case_1() {
        ...
        assert!(sha256::verify(&expected_dk, &password_256, &salt, iter, &mut dk_out).is_ok());
        assert!(sha384::verify(&expected_dk, &password_384, &salt, iter, &mut dk_out).is_ok());
        assert!(sha512::verify(&expected_dk, &password_512, &salt, iter, &mut dk_out).is_ok());
    }
